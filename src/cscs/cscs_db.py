...
class CSCGDB():
    ...
    def get_ability(session, ab_id:int) -> CSAbilityLang:
        ...
        return session.scalars(select(CSAbilityLang).join(Language).join(CSAbility).where(CSAbility.enabled==True).where(Language.id==settings.LANG_ID).where(CSAbility.id==ab_id))
